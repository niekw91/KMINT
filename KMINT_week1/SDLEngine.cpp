...
void SDLEngine::RenderImage(GameObject *obj) {
	SDL_Rect rect;
	rect.h = obj->image->h / 4;
	rect.w = obj->image->w / 4;
	rect.x = obj->current->x - rect.w / 2;
	rect.y = obj->current->y - rect.h / 2;

	SDL_RenderCopy(renderer, SDL_CreateTextureFromSurface(renderer, obj->image), NULL, &rect);
}
